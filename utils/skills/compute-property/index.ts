...
export const skill = {
  ...
  description: 'Calculate or aggregate property values (sum, avg, min, max) from BIM elements. Can compute area from width/height if missing.',
  parameters: {
    type: 'object',
    properties: {
      property: {
        type: 'string',
        description: 'Property name to compute (e.g., "Area", "Height", "Volume")',
      },
      operation: {
        type: 'string',
        enum: ['list', 'sum', 'avg', 'min', 'max', 'count'],
        description: 'Operation to perform: list (get all values), sum, avg, min, max, or count',
      },
      ids: {
        type: 'array',
        items: { type: 'string' },
  ...
  execute: async (params: { property: string; operation: string; ids: string[] }) => {
