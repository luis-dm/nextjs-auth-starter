...
const defaultConfig: Required<SmoothWheelControlConfig> = {
  ...
  proximitySlowdown: false, // Automatic speed adjustment based on object distance
...
export function createSmoothWheelControl(
  ...
  // const raycastTimeoutId: number | null = null;
  ...
  const wheelHandler = (e: WheelEvent) => {
    ...
    const isTrackpadPinch = e.ctrlKey && Math.abs(e.deltaY) > 0;

    const isMouseWheel = e.deltaMode === 1;

    const isTrackpadScroll =
      e.deltaMode === 0 && !e.ctrlKey && Math.abs(e.deltaY) > 5;

    if (isTrackpadPinch) {
      dynamicStepValue = TRACKPAD_PINCH_STEP;
    } else if (isMouseWheel) {
      dynamicStepValue = MOUSE_WHEEL_STEP;
    } else if (isTrackpadScroll) {
      dynamicStepValue = TRACKPAD_SCROLL_STEP;
    } else {
      dynamicStepValue = DEFAULT_STEP;
    }
    ...
    const base = dynamicStepValue; // Use detected input type step value
