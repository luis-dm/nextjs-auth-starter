...
interface FirstPersonConfig {
  ...
  ) => void; // Callback for note creation
}
  ...
  if (highlighter && result && result.localId !== undefined) {
    try {
      ...
      console.log("Element highlighted:", {
        localId: result.localId,
        name: elementName,
        // Don't log the full model object - just indicate it exists
        hasModel: !!model,
      });
    } catch (error) {
...
const onNoteCreate = async () => {
  ...
  const { result, model } = raycastEntry;
  // Only log serializable properties
  console.log("Creating note at raycast result:", {
    localId: result.localId,
    distance: result.distance,
    point: result.point
      ? { x: result.point.x, y: result.point.y, z: result.point.z }
...
const animate = () => {
  ...
  if (pointerLockControls.isLocked === true) {
    ...
    if (moveBackward)
      movement.add(forward.clone().multiplyScalar(-moveSpeed * delta));
    if (moveRight)
      movement.add(right.clone().multiplyScalar(moveSpeed * delta));
    if (moveLeft)
      movement.add(right.clone().multiplyScalar(-moveSpeed * delta));
