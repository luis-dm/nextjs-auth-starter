...
A generalized cast function that traverses two BVH structures simultaneously to perform intersection tests between them. This is used internally by intersectsGeometry. The function returns true as soon as a triangle pair has been reported as intersected by the callbacks. Both BVH trees are traversed in depth-first order, alternating descent.
...
/_ update the positions of the triangle vertices _/
...
See note in Asynchronous Generation use snippet.
...
Set of shaders structs and defined constants used for interacting with the packed BVH in a shader. See src/webgl/glsl/bvh_struct_definitions.glsl.js for full implementations and declarations.

shaderFunctions
BVHShaderGLSL.bvh_distance_functions : string
BVHShaderGLSL.bvh_ray_functions : string
BVHShaderGLSL.common_functions : string
Set of shader functions used for interacting with the packed BVH in a shader and sampling VertexAttributeTextures. See src/webgl/glsl for full implementations and declarations.
